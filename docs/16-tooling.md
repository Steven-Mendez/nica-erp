...
Style rules that apply across the codebase. Architectural rules (layering, ports, events) live in [02 — Architecture](02-architecture.md); these are the **how-to-write-it** rules.
...
Default: **no comment**. The well-named identifier already says what the code does — don't restate it.
...
def issue_invoice(draft: Invoice) -> Invoice:
    """Issue an invoice and return the issued invoice."""
```

Add a comment only when it captures **WHY** — a hidden constraint, a non-obvious invariant, a workaround, or behavior that would surprise a future reader.

```python
# Good — captures a constraint the reader cannot infer
# DGI requires the issue date to be in America/Managua wall-clock time,
# not UTC, so the sequence date matches the printed invoice header.
issue_date = now_managua().date()
```

Avoid:
- References to the current PR, ticket, or commit (`# fixes #423`, `# for the new onboarding flow`) — that context belongs in the PR description and rots fast in the file.
- "Used by X" comments — `git grep` is authoritative; the comment will lie within a week.
- TODO comments without an owner and a removal condition. Prefer a tracked issue.

### Docstrings

- One short line for trivial functions; skip entirely if the name + types are self-explanatory.
- 2–5 lines for use cases, domain services, and ports — explain **purpose and side-effects**, not signature.
- Module-level docstrings on every non-trivial module explain what the module owns.

```python
# Good
def issue_invoice(draft: Invoice, sequences: NumberSequencePort) -> Invoice:
    """
    Allocate the next DGI number for the document type, stamp the invoice
    as issued, and emit InvoiceIssued. Idempotent under the same draft_id +
    idempotency_key.
    """

# Bad — restates the signature
def issue_invoice(draft: Invoice, sequences: NumberSequencePort) -> Invoice:
    """Take a draft and a sequence port, return an invoice."""
```

### What not to write as a file

- **Planning / decision / analysis docs** — work from conversation context, not intermediate files. ADRs are for architectural decisions; everything else lives in the PR description.
- **Summary docs of recent changes** — that's what the commit log and PR description are for.
- **README files inside subdirectories** — unless the directory is a deliverable in itself. Architecture lives here in `docs/`, not scattered READMEs.

### Python style essentials

**Type hints (Python 3.13).** Required in `domain/`, `application/`, and `bootstrap/` (mypy `--strict`). Recommended in adapters.

```python
# Good — modern union and generics
def issue_invoice(draft_id: InvoiceId, idempotency_key: str | None = None) -> Invoice:
    ...

# Bad — legacy typing
from typing import Optional
def issue_invoice(draft_id: InvoiceId, idempotency_key: Optional[str] = None) -> "Invoice":
    ...
```

**Imports.** Sorted by `ruff` (`I` rule). Don't fight it.

**Async.** Use `async def` for I/O. Don't mix sync drivers with the async session.

**Errors.** Raise domain exceptions from `domain/`; adapters translate to HTTP per [08 — API Conventions §Errors](08-api-conventions.md#errors--rfc-7807-problem-details). Don't catch `Exception` to "make tests pass" — handle the specific class or let it bubble.

### TypeScript / React style essentials

- **`strict: true`** in `tsconfig.json` — no `any` without justification (use `unknown` + narrowing). ESLint enforces.
- **API client** is generated by `pnpm gen:api` from the live OpenAPI spec — **never** hand-edit `src/api/schema.d.ts`.
- **Forms** use TanStack Form + Zod for validation; never call `fetch` directly — go through `openapi-fetch`.
- **Components** are typed function components. Props are an explicit `interface`, not inline.
- **State** lives in TanStack Query (server) or component state (UI); avoid global state unless you have a reason.
...
| Private        | Leading underscore   | `_allocate_number()`               |
| Type aliases   | `PascalCase`         | `InvoiceId = NewType("InvoiceId", UUID)` |

**TypeScript / React:**

| Type           | Convention           | Example                            |
| -------------- | -------------------- | ---------------------------------- |
| Files (component) | `PascalCase.tsx`  | `InvoiceForm.tsx`                  |
| Files (other)  | `camelCase.ts`       | `formatCurrency.ts`                |
| Hooks          | `useCamelCase`       | `useIssueInvoice`                  |
...
**Domain events.** `<context>.<event>` in past tense: `sales.invoice_issued`, `inventory.stock_adjusted`, `iam.user_invited`. Payload schema versioned via `event_schema_version`.
