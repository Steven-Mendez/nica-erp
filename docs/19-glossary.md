...
| **EventBridge** | Event bus. Custom bus `nica-erp` with rules to SQS. |
...
| **CloudFront** | AWS CDN. Under [ADR-0020](adr/0020-no-custom-domain-mvp.md) it is the **only HTTPS front-door** via `https://<dist-id>.cloudfront.net/` with two behaviors: `/*` → S3 `nica-erp-web`, `/api/*` → ALB (HTTP-only, no cache). |
...
| **Project naming** | **`nica-erp`** everywhere by convention: GitHub repo and branding, distribution package (`pyproject.toml` `name = "nica-erp"`), npm package (`@nica-erp/web`), and AWS resource prefix (`nica-erp-api`, `nica-erp-files`, `/nica-erp/db/master`, `nica-erp.auth.us-east-1.amazoncognito.com`). Python source has **no wrapping package**: top-level imports are `bootstrap`, `shared_kernel`, `contexts` directly under `apps/api/src/` ([`pyproject.toml` `[tool.hatch.build.targets.wheel] packages`](../apps/api/pyproject.toml)). The Postgres role/db is `nica_erp` (underscore) since Postgres identifiers can't contain hyphens. |
