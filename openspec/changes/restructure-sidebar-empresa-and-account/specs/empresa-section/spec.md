...
## Backend gaps surfaced but NOT implemented in this change

### Gap: Per-user permission overrides (deferred)

The existing RBAC catalog
(`apps/api/src/shared_kernel/permissions/catalog.py`) maps roles
to fixed permission sets. To support granular per-user grants
beyond role membership, the backend SHALL eventually add:

- A new table `tenant_member_permissions` with columns
  `(tenant_id, user_id, permission_code, granted, created_at)`,
  primary key `(tenant_id, user_id, permission_code)`, RLS policy
  matching the canonical `tenant_isolation` pattern from sprint
  03.
- An Alembic migration creating the table with no seed rows
  (every member's effective set defaults to the role catalog
  until an override is written).
- A new permission code `members:update-permissions` (the
  operator's right to assign granular grants) seeded into the
  catalog for the `owner` + `admin` roles.
- Endpoints
  `GET /v1/tenants/{id}/members/{user_id}/permissions` and
  `PUT /v1/tenants/{id}/members/{user_id}/permissions` returning
  / accepting `{ grants: [{code, granted}], …}`. Both gated by
  `members:update-permissions`.
- An `Actor.permissions` resolver that unions the role catalog
  with the per-user overrides.

This change does NOT implement the gap. The
`Empresa → Usuarios` page surfaces role changes only; the
deferred grants form is out of scope.

### Gap: `POST /v1/invitations/accept` does not set `app.tenant_id`

Tracked under [[test-backfill-and-e2e-tooling]] §3.2. The
owner-facing UI in this change does not depend on the accept
flow; the invitee's accept route stays untouched and continues to
fail at runtime until the GUC fix lands.
