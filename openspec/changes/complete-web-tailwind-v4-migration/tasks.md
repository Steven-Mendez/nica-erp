...
- [ ] 1.1 Run `pnpm --filter @nica-erp/web build` on a clean working tree;
...
- [ ] 2.1 Update `apps/web/package.json`:
...
- [ ] 4.1 Move the `theme.extend.colors` map from
...
- [ ] 4.2 Move the `theme.extend.borderRadius` map into the same
      `@theme inline {}` block:
      `--radius-lg: var(--radius);`,
      `--radius-md: calc(var(--radius) - 2px);`,
      `--radius-sm: calc(var(--radius) - 4px);`.
- [ ] 4.3 Shrink `tailwind.config.ts` to
...
- [ ] 4.4 Run `pnpm --filter @nica-erp/web build` and `diff` the new
...
- [ ] 5.1 `card.tsx`, `dialog.tsx`, `popover.tsx`, `dropdown-menu.tsx`,
      `select.tsx`: replace `shadow-sm` with `shadow-xs`. Re-run
      `pnpm test:run` after each file.
- [ ] 5.2 `checkbox.tsx`, `badge.tsx`, `dialog.tsx` (close button):
      replace `rounded-sm` with `rounded-xs`.
- [ ] 5.3 `button.tsx`, `input.tsx`, `textarea.tsx`, `command.tsx`,
      `select.tsx`: replace `outline-none` with `outline-hidden`.
- [ ] 5.4 `button.tsx` (focus-visible), `input.tsx`: replace
      `ring-2 ring-offset-2` with `ring-3`.
- [ ] 5.5 `dialog.tsx` (overlay), `popover.tsx` (overlay): replace
      `bg-opacity-50` with the colour-slash form (e.g.
      `bg-black/50`).
- [ ] 5.6 `dropdown-menu.tsx`, `command.tsx`: replace
      `data-[state=open]:bg-accent` with
      `data-[state=open]:bg-accent/50` to match the v4 translucent surface
      convention.
- [ ] 5.7 Grep for the deprecated utilities listed in the v4 upgrade guide
...
- [ ] 6.1 Re-run `pnpm --filter @nica-erp/web test:e2e`; the screenshot diff
      ...
      routes — 8 diffs total).
- [ ] 6.2 Re-run the axe a11y scan; violation count must equal the §1.3
      baseline.
- [ ] 6.3 Run `pnpm dlx shadcn@latest add accordion --dry-run` from
...
- [ ] 6.4 Run `pnpm --filter @nica-erp/web typecheck && pnpm --filter
...
- [ ] 7.1 Amend [ADR-0009](../../../docs/adr/0009-frontend-stack.md) with a
...
- [ ] 7.2 Update [`docs/09-frontend.md`](../../../docs/09-frontend.md) the
...
- [ ] 7.3 No sprint document changes — this migration is infrastructure,
