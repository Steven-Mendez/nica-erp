...
- [ ] 1.1 New value object
      ...
      `offset >= 0`, `len(q) <= 200`.
- [ ] 1.2 New result VO `ListMembersResult(items: list[Membership],
      total: int)`.
- [ ] 1.3 Update `ListMembers.execute(query)` to accept the VO and
      return `ListMembersResult`. Old positional signature gone.
- [ ] 1.4 Unit test for the use case: happy path delegates to the
      repository; param validation surfaces as a `ValueError` /
      domain error.

## 2. Persistence

- [ ] 2.1 `MembershipRepository` port (Protocol) gains
...
- [ ] 2.2 SqlAlchemy adapter `MembershipRepositorySqlAlchemy.list_page`
...
- [ ] 2.3 `q` is applied as a single OR predicate:
      ...
      in user input.
- [ ] 2.4 Sort whitelist enforced in adapter (never interpolate
...
- [ ] 2.5 The COUNT query applies the same WHERE predicate (no
...
- [ ] 3.1 `MembersPageResponse` Pydantic schema:
      `{ items: list[MemberResponse], total: int, limit: int, offset: int }`.
- [ ] 3.2 Router `list_members` accepts `Query` params: `limit=25`,
...
- [ ] 3.3 Update
      ...
      `ListMembersResult`.
- [ ] 3.4 Add a router test for: defaults round-trip, `limit > 100`
      returns `422`, `q` shorter than 200 chars accepted.

## 4. OpenAPI + types

- [ ] 4.1 `make api` boots, `GET /openapi.json` validates.
- [ ] 4.2 `cd apps/web && pnpm gen:api`; commit the regenerated
      `apps/web/src/api/schema.d.ts`.

## 5. Frontend endpoints + hooks

- [ ] 5.1 `apps/web/src/features/tenants/api/endpoints.ts`:
      `listMembers(tenantId, params)` returns the new envelope; type
      `Member` is unchanged but new `MembersPage` shape exported.
- [ ] 5.2 `apps/web/src/features/tenants/api/hooks.ts`:
...
- [ ] 6.1 `apps/web/src/routes/empresa/users.tsx` derives a
...
- [ ] 6.2 `MembersTable` accepts `total` (number) and switches the
...
- [ ] 6.3 The "Limpiar" button still resets the in-component view
      state but now the change propagates up to the route, which
      writes URL params, which re-runs the query.
- [ ] 6.4 The result counter (`{n} resultado(s)`) reads from
...
- [ ] 7.1 `pytest -m "unit or integration"` exit 0.
- [ ] 7.2 `pnpm typecheck`, `pnpm lint --max-warnings=0`,
      `pnpm test --run` exit 0.
- [ ] 7.3 `make migrate && make migrate-down && make migrate` clean
      (migration `0005` already merged; no new migration in this
      change).
- [ ] 7.4 `scripts/smoke-api.sh` exit 0; SPA reload at
