...
`AuthLockoutActive(retry_after_seconds=…, scope=…)`. The check uses
...
- **THEN** `AuthenticateUseCase.execute` raises `AuthLockoutActive` and never invokes the password-compare adapter
...
### Requirement: HTTP router maps AuthLockoutActive to 429 + auth.lockout_active problem + Retry-After

The inbound HTTP router SHALL handle `AuthLockoutActive` exceptions raised by `AuthenticateUseCase` and respond:
