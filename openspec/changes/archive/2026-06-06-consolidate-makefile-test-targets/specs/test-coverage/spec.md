...
#### Scenario: `make test SCOPE=be COV=1` fails when a module drops below the gate
...
- **THEN** `make test SCOPE=be COV=1` SHALL exit non-zero and the
...
`make test SCOPE=fe COV=1` measures
...
#### Scenario: `make test SCOPE=fe COV=1` fails when a new component lands without tests
...
- **THEN** `make test SCOPE=fe COV=1` SHALL exit non-zero and the
...
default `make test` invocation (which exercises all three backend
lanes).
