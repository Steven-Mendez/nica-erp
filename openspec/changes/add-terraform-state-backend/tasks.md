...
- [ ] 1.1 Create directory `infra/terraform/bootstrap/` and add
      `infra/terraform/.gitignore` blocking `.terraform/`,
      `*.tfstate`, `*.tfstate.*`, `terraform.tfvars`,
      `.deploy-image-tag`.
- [ ] 1.2 Author `infra/terraform/bootstrap/versions.tf` pinning
      Terraform `>= 1.6` and `hashicorp/aws ~> 5.0`.
- [ ] 1.3 Author `infra/terraform/bootstrap/providers.tf` declaring
...
- [ ] 2.2 Add a bucket policy on `nica-erp-tf-state` denying any
      request whose `aws:SecureTransport` is `false`.
- [ ] 2.3 Create `aws_dynamodb_table.tf_lock` named `nica-erp-tf-lock`
      with `billing_mode = "PAY_PER_REQUEST"`, hash key `LockID`
      (string), explicit tag `Project=nica-erp`.
- [ ] 2.4 Expose outputs `tf_state_bucket`,
      `tf_state_bucket_arn`, `tf_lock_table`,
      `tf_lock_table_arn`.

## 3. Image registry (ECR)

- [ ] 3.1 Author `infra/terraform/bootstrap/ecr.tf`: create
...
- [ ] 3.2 Attach `aws_ecr_lifecycle_policy.api` with rule selecting
      `tagStatus=any`, `countType=imageCountMoreThan`,
      `countNumber=5`, action `expire`.
- [ ] 3.3 Expose output `ecr_repository_url`.

## 4. SPA bucket and CloudFront distribution

- [ ] 4.1 Author `infra/terraform/bootstrap/web.tf`: create
...
- [ ] 4.2 Create `aws_cloudfront_origin_access_control.web` with
      `signing_protocol="sigv4"`, `signing_behavior="always"`,
      `origin_access_control_origin_type="s3"`.
- [ ] 4.3 Create `aws_cloudfront_distribution.web` with:
...
- [ ] 5.2 Author `scripts/destroy-bootstrap.sh` (bash,
      `set -euo pipefail`):
...
- [ ] 5.3 Add `Makefile` targets `bootstrap` and `destroy-bootstrap`
...
- [ ] 6.2 Upload a placeholder `index.html` with
      `aws s3 cp index.html s3://nica-erp-web/index.html`, then curl
      `https://<dist>.cloudfront.net/` and confirm HTTP 200 + the
      placeholder body.
...
- [ ] 6.5 Run `terraform -chdir=infra/terraform/bootstrap plan` a
      second time and confirm `No changes` (accepting documented
      `default_tags` / S3 ETag caveats).
...
- [ ] 6.7 Run `make destroy-bootstrap`, type `nica-erp-bootstrap`,
      confirm buckets and ECR are emptied and the five Terraform
      resources are gone.
- [ ] 6.8 Run
      `aws resourcegroupstaggingapi get-resources --tag-filters Key=Project,Values=nica-erp`
      and confirm an empty result.
