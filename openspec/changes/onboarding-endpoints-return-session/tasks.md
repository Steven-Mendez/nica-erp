...
- [ ] 1.1 Extend `ConfirmSignupRequest` Pydantic schema in
...
- [ ] 1.2 Update the `ConfirmSignup` use case at
      ...
      current `None`.
- [ ] 1.3 Update the FastAPI route at
...
- [ ] 2.1 Extend `AcceptInvitationRequest` Pydantic schema in
...
- [ ] 2.2 Update `AcceptInvitation` use case at
...
- [ ] 2.3 Update the FastAPI route at
...
- [ ] 3.1 In `apps/web/src/routes/signup.tsx`, after a successful
      `POST /v1/auth/register`, navigate to `/confirm` with the
      typed password in TanStack Router state (not in the URL).
- [ ] 3.2 In `apps/web/src/routes/confirm.tsx`, read the password
...
- [ ] 3.3 Update `ConfirmSignupInput` and the mutation hook in
      ...
      narrowed branch.
- [ ] 3.4 Frontend unit / integration tests for `confirm.tsx` cover
...
- [ ] 4.1 In `apps/web/src/features/tenants/api/endpoints.ts`,
...
- [ ] 4.2 In `apps/web/src/routes/invitations/accept.tsx`, after a
...
- [ ] 4.3 Frontend unit / integration tests for
...
- [ ] 5.4 Run `pnpm e2e -- --grep="invitation|signup"` locally
      against the make-api stack to confirm all three specs pass.

## 6. Wrap-up

- [ ] 6.1 `make api-test` and `pnpm test --run` and
      `pnpm typecheck` exit 0.
- [ ] 6.2 `pnpm gen:api` regenerates `apps/web/src/api/schema.d.ts`
      with the new optional fields; commit the regen separately.
- [ ] 6.3 Verify the seamless flow manually in `make api` + `pnpm dev`
      against Mailpit: register, copy code, confirm — the SPA lands
      authenticated on the invited dashboard without any extra step.
- [ ] 6.4 Update the OpenSpec change status to "ready for archive"
